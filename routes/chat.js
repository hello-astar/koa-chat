 ...
 * @LastEditors: astar
 * @LastEditTime: 2021-05-07 21:22:08
 * @Description: 文件描述
...
router.get('/getRecentContacts', async ctx => {
  ctx.verifyParams({
  ...
  let res = await chatController.getRecentContacts({ userId, pageNo: Number(pageNo), pageSize: Number(pageSize) });
  ctx.send(res);
});
