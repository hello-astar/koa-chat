 ...
 * @LastEditors: astar
 * @LastEditTime: 2021-04-21 18:57:02
 * @Description: 文件描述
    ...
    return this.Model.find({ members: { $in: receiverId } })
                    .populate({
                      ...
                      return await Promise.all(res.map(async item => {
                        ...
                        const composite = await (avatarBuffers.reduce((input, overlay) => {
                        ...
                        let last = await sharp(composite).png().toBuffer()
                        return {
                          _id: item._id,
                          groupName: item.groupName,
                          addTime: item.addTime,
                          avatar: 'data:image/png;base64,'+ last.toString('base64')
                        }
                      }));
                    });
  }
